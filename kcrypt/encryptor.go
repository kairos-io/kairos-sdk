...
func (e *RemoteKMSEncryptor) getPasswordFromChallenger(b *partitions.Partition) (password string, err error) {
	...
	log := logger.NewKairosLogger("kcrypt-getPassword", "info", false)
	defer log.Close()
...
func GetEncryptor(logger logger.KairosLogger) (PartitionEncryptor, error) {
...
func scanCollectorConfig(logger logger.KairosLogger) *collector.Config {
...
func detectUKIMode(logger logger.KairosLogger) bool {
...
func validateSystemdVersion(logger logger.KairosLogger, minVersion int) error {
...
func validateTPMDevice(logger logger.KairosLogger) error {
