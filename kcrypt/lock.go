...
func encryptWithLocalTPMPassphrase(label string, nvIndex, cIndex, tpmDevice string, logger logger.KairosLogger, argsCreate ...string) (string, error) {
...
func luksifyWithPassphrase(label string, passphrase string, logger logger.KairosLogger, argsCreate ...string) (string, error) {
...
func unmountIfMounted(device string, logger logger.KairosLogger) error {
...
func luksifyMeasurements(label string, publicKeyPcrs []string, pcrs []string, logger logger.KairosLogger, argsCreate ...string) error {
	if err := udevAdmSettle(&logger, udevTimeout); err != nil {
		return err
	}
...
func formatLuks(device, name, mapper, label, pass string, logger logger.KairosLogger) error {
...
func udevAdmSettle(logger *logger.KairosLogger, timeout time.Duration) error {
	if logger != nil {
